...
// 创建错误
err := errorx.NewError(sqlbuilder.ErrorCodeNotFound)

// 带消息的错误
err := errorx.NewError(sqlbuilder.ErrorCodeInvalidInput).
    WithMessage("用户名不能为空")

// 检查错误类型
if errorx.IsType(err, sqlbuilder.ErrorCodeNotFound) {
    // 处理未找到的情况
}
```
...
if err != nil {
    if errors.Is(err, gorm.ErrRecordNotFound) {
        // 记录不存在
        return nil, errorx.NewError(sqlbuilder.ErrorCodeNotFound).
            WithMessage("用户不存在")
    }
...
type UserRepository struct {
    *sqlbuilder.BaseRepository[User]
}

func (r *UserRepository) GetByEmail(ctx context.Context, email string) (*User, error) {
    user, err := r.GetByFilter(ctx, sqlbuilder.NewEqFilter("email", email))
    if err != nil {
        if errors.Is(err, gorm.ErrRecordNotFound) {
            return nil, errorx.NewError(sqlbuilder.ErrorCodeNotFound).
...
func (r *UserRepository) CreateUser(ctx context.Context, user *User) (*User, error) {
    // 验证输入
    if user.Username == "" {
        return nil, errorx.NewError(sqlbuilder.ErrorCodeInvalidInput).
            WithMessage("用户名不能为空")
    }
    ...
    // 检查邮箱是否已存在
    exists, err := r.Exists(ctx, sqlbuilder.NewEqFilter("email", user.Email))
    if err != nil {
        return nil, err
    }
...
func (s *UserService) Register(ctx context.Context, req *RegisterRequest) error {
    ...
    if err != nil {
        ...
        if errors.Is(err, gorm.ErrDuplicatedKey) {
            return errorx.NewError(sqlbuilder.ErrorCodeInvalidInput).
...
func (s *OrderService) CreateOrder(ctx context.Context, order *Order, items []*OrderItem) error {
    return s.repo.Transaction(ctx, func(tx sqlbuilder.Transaction[Order]) error {
        // 创建订单
...
func (r *UserRepository) Login(ctx context.Context, username, password string) (*User, error) {
    user, err := r.GetByFilter(ctx, sqlbuilder.NewEqFilter("username", username))
    if err != nil {
    ...
    if !checkPassword(user.Password, password) {
        ...
        return nil, errorx.NewError(sqlbuilder.ErrorCodeForbidden).

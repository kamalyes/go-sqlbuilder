 ...
 * @LastEditTime: 2025-11-23 22:39:16
 * @FilePath: \go-sqlbuilder\repository.go
 * @Description:
 *
...
func (r *BaseRepository[T]) Create(ctx context.Context, entity *T) (*T, error) {
	if r.readOnly {
		return nil, errorx.NewError(ErrorCodeForbidden)
	}

	if entity == nil {
		return nil, errorx.NewError(ErrorCodeInvalidInput)
	}
...
func (r *BaseRepository[T]) CreateIfNotExists(ctx context.Context, entity *T, uniqueFields ...string) (*T, bool, error) {
	if r.readOnly {
		return nil, false, errorx.NewError(ErrorCodeForbidden)
	}
...
func (r *BaseRepository[T]) Transaction(ctx context.Context, fn func(tx Transaction[T]) error) error {
	return r.db.GetDB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		txHandler, err := NewGormHandler(tx)
		if err != nil {
			return err
		}
...
func applyFilter(dbQuery *gorm.DB, filter *Filter) *gorm.DB {
	...
	case OP_EQ, OP_NEQ, OP_GT, OP_GTE,
		OP_LT, OP_LTE, OP_IN, OP_NOT_IN,
		OP_LIKE, OP_NOT_LIKE:
...
func buildFilterCondition(filter *Filter) (string, interface{}) {
	...
	case OP_BETWEEN:
		if values, ok := filter.Value.([]interface{}); ok && len(values) == 2 {
			return filter.Field + " BETWEEN ? AND ?", values
		}
		return "", nil
	case OP_FIND_IN_SET:
		return "FIND_IN_SET(?, " + filter.Field + ")", filter.Value
	default:
		return "", nil
	}
}
